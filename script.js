...
function getTagCounts(dataset) {
    const counts = {};
    dataset.forEach((item) => {
        const tags = item.tags ? item.tags.split(",").map((t) => t.trim()).filter((t) => t.length > 0) : ["Uncategorized"];
...
function exportResults(type = "csv") {
    const selectedData = selectionMode && selectedItems.size > 0 ? filtered.filter((item) => selectedItems.has(item.url)) : filtered;
    if (selectedData.length === 0) {
    ...
    if (type === "markdown") {
        const md = selectedData
            .map(item => {
                const cleanUrl = item.url.split('?')[0]; // pulisco URL togliendo query
                return `[${item.title}](${cleanUrl})`;
            })
            .join('\n');
        copyToClipboard(md);
...
function adjustExportMenuPosition() {
    const container = document.querySelector('.export-dropdown-container');
    const menu = container.querySelector('.export-menu');

    // Reset per calcolare correttamente
    menu.classList.remove('left');

    const containerRect = container.getBoundingClientRect();
    const menuWidth = menu.offsetWidth;

    // Spazio a sinistra dalla viewport
    const spaceOnLeft = containerRect.left;

    // Se non c'è abbastanza spazio a sinistra per il menu
    if (spaceOnLeft < menuWidth) {
        menu.classList.add('left');
    }
}

// Esegui la funzione all'apertura del menu o al resize della finestra
window.addEventListener('resize', adjustExportMenuPosition);

// Esempio di chiamata quando apri il menu:
document.querySelector('.export-dropdown-container').addEventListener('click', () => {
    adjustExportMenuPosition();
});